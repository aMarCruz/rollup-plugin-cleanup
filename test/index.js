...
describe('Issues', function () {
  ...
  it('must handle .jsx files (issue #1)', function () {
    ...
    return rollup(opts).then((bundle) =>
      bundle.generate({ format: 'cjs', exports: 'auto' }).then((result) => {
        expect(result.output).toBeAn(Array)
        const output = result.output[0]
        expect(output).toExist()
        expect(output.code).toMatch(/module\.exports/)
        expect(output.code).toNotMatch(/to_be_removed/)
        expect(output.map).toNotExist()
      })
    )
  })
